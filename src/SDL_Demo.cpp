...
int main(int argc, char* argv[]) {
    ...
    std::vector<VideoFrame> videoFrames;  // 视频帧集合
    Uint32 videoStartTime = 0;           // 视频播放开始时间
    ...
    const char* settingsPath = u8"SETINGS.png";//SETING
    ...
    GameObject player = {
       playerTexture,
       {100, GROUND_Y , 50, 50}, // 初始位置在地面上（地面 Y=470，角色高 80）
       0,
       0,
       false
    };
    ...
    while (isRunning) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (currentState == OPTIONS && event.type == SDL_MOUSEBUTTONDOWN) {
                ...
                for (int i = 0; i < 4; ++i) {
                    if (SDL_PointInRect(&mousePos, &optionRects[i])) {
                        ...
                        case 2: currentState = SETTINGS; break;//SETINGS
                            // 修改选项点击处理
                        case 3:
            ...
            case EXIT_SCREEN:
             // 显示3秒后退出
                 if (SDL_GetTicks() - transitionStartTime > 3000) {
                     isRunning = false;
                 }
                  break;
                  // 在状态更新部分添加（主循环内）
            case CUTSCENE: {
                ...
                if (elapsed > 2000) {
                    ...
                    if (currentCutsceneIndex >= cutsceneTextures.size()) {
                        ...
                        gameMapTexture = IMG_LoadTexture(renderer, "D:\\path\\to\\map2.png");
                        currentState = GAME;
            ...
            if (currentState == GAME) {
                ...
                player.speedX = 0;
                if (keystates[SDL_SCANCODE_LEFT]) player.speedX = -5;  // 左方向键
                else if (keystates[SDL_SCANCODE_RIGHT]) player.speedX = 5; // 右方向键
                player.position.x += player.speedX;
                player.collider.x = player.position.x + 10;
                SDL_Log("Player X: %d", player.position.x);
                if (event.type == SDL_KEYDOWN) {
                    SDL_Log("按键按下: %s", SDL_GetScancodeName(event.key.keysym.scancode));
                }
                ...
                // 跳跃逻辑
                if (keystates[SDL_SCANCODE_SPACE] && player.position.y >= GROUND_Y - 50) {
                    player.speedY = -15;
